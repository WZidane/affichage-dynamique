...
onMounted(() => {
    ...
    session.setNav();
    getDeviceInformation(); // Appel initial pour récupérer les données une première fois
    setInterval(getDeviceInformation, 30 * 60 * 1000); // Appel toutes les 30 minutes
})
...
function getDeviceInformation() {
    ...
    axios.get(`${token.state.BASE}${token.state.OBJ}?filter[id][_eq]=0593540e-7f04-4f4b-87ff-30e2d2835839&fields=
id,
...
Sequences.Sequence_id.Ecrans.Ecran_id.Template.background_color.color`).catch((response) => {
        if ((response.response.data.errors[0].extensions.code === 'TOKEN_EXPIRED')) {
            axios.post(`https://74b3jzk3.directus.app/auth/refresh`, {
                refresh_token: token.state.REFRESHUSER,
                mode: 'json'
            }).then(response => {
